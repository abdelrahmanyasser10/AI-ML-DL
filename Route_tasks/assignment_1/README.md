...
[cite_start]**Assignment Name**: Introduction to Programming [cite: 1]
**Course**: Machine Learning with Route Academy
**Instructor**: Abdelrahman Yasser Mohamed

[cite_start]This assignment covers a range of introductory Python programming problems designed to build a strong foundation in core concepts like control flow, data structures, and basic algorithms[cite: 1].
...
[cite_start]**Description**: Find the maximum and minimum of three numbers, or check if they're equal and determine if the number is positive or negative[cite: 2].
...
[cite_start]**Description**: Check if a word contains a vowel and print its position if one is found[cite: 5, 6].
...
[cite_start]**Description**: Display the last digit of a given number[cite: 7].
**Approach**: The modulo operator (`%`) is used with 10 to extract the last digit of the number.

### Problem 4: Divisibility Check
[cite_start]**Description**: Determine if a number is divisible by both 2 and 3[cite: 8].
...
[cite_start]**Description**: Calculate a student's attendance percentage and determine if they can sit for an exam if their percentage is less than 75%[cite: 9].
...
[cite_start]**Description**: Convert a string to all uppercase or all lowercase based on whether lowercase letters are less than uppercase, or vice versa[cite: 12, 13, 14].
...
[cite_start]**Description**: Check if a number is a palindrome or an Armstrong number[cite: 15].
...
[cite_start]**Description**: Convert a number to its word representation (e.g., 124 → One hundred and twenty-four)[cite: 16].
...
[cite_start]**Description**: Calculate the sum of the series $1 + 4 - 9 + 16 - 25 + 36 ... n$ terms[cite: 19].
...
[cite_start]**Description**: Count the occurrences of the word "car" in a string and find their positions[cite: 20].
...
[cite_start]**Description**: Process user input differently based on its type: sum digits for a number, remove spaces for text, or split numbers and text if both are present[cite: 22, 23, 24, 25].
...
[cite_start]**Description**: Calculate the average of numbers within each inner tuple of a tuple of tuples[cite: 26].
...
[cite_start]**Description**: Check if a list is empty and, if not, sort it in increasing order[cite: 29, 30].
...
[cite_start]**Description**: Remove all even numbers from a specified list[cite: 31, 32].
...
[cite_start]**Description**: Extract elements from a list that have a frequency greater than a specified value `K`[cite: 33].
...
[cite_start]**Description**: Sort both the values (lists) and the keys of a dictionary[cite: 34].
...
[cite_start]**Description**: Create and manage a dictionary of student information (age and grades) with options to view and edit student data[cite: 36, 37].
